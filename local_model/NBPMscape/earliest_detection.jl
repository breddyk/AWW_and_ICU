# ============================================================================
# earliest_detection.jl
#
# Single-scenario driver: ICU + AWW + HARISS.
# Returns only the EARLIEST detection time across all three channels, the
# channel that achieved it (AWW < ICU < HARISS on ties), and local cases
# (generation > 0) at that moment.
#
# Performance improvements over multitype_comparison.jl:
#
#   1. hariss_bg_cache is built ONCE on the main process before pmap. All
#      samples share one realisation of the background ARI hospital
#      population. The background is independent of the epidemic trajectory,
#      so sharing is statistically defensible and saves N-1 expensive
#      build_ari_background calls (the identified bottleneck vs. AWW+ICU).
#
#   2. sampleforest is called ONCE per sample after the loop, not every
#      day. The daily call in multitype_comparison.jl is O(n²) in total
#      sampleforest cost as sample_infections grows. A single post-loop
#      call is O(n).
#
#   3. The epidemic tree is only built while AWW has not yet detected. Any
#      ICU or HARISS detection before AWW is captured in the truncated tree;
#      detections after AWW cannot be the earliest so the extra tree cost is
#      wasted. When AWW never fires the tree is built to max_observation_time.
#
#   4. CachingPool avoids re-serialising hariss_bg_cache to each worker on
#      every pmap task.
# ============================================================================
...
@everywhere function simulate_earliest_detection(
    country_data::DataFrame,
    country_name::String,
    sample_id::Int,
    R0::Float64,
    ...
    for (idx, row) in enumerate(eachrow(country_data))
        ...
        # AWW: always run — cheap Bernoulli draws, no tree required.
        ...
        # Tree: only needed for ICU/HARISS detections that could beat AWW.
        # Once AWW fires, stopping here is correct: any ICU or HARISS detection
        # from later imports would arrive after t_aww, so it cannot be earliest.
        if !aww_detected
    ...
    # ICU: single sampleforest on whatever tree was built.
    # Captures all ICU detections that could precede AWW.
    t_icu = Inf
    if !isempty(sample_infections)
        icu_sampled = NBPMscape.sampleforest((G = sample_infections,), icu_params)
        if !isempty(icu_sampled.treport)
            t_icu = minimum(icu_sampled.treport)
        end
    end

    # HARISS: single call using the pre-built shared background cache.
    t_har = Inf
    ...
    t_aww = first_aww_time
    t_min = min(t_icu, t_har, t_aww)

    # Tie-break: AWW < ICU < HARISS
    ...
    local_cases = NaN
    if !isempty(sample_infections) && t_min < Inf
        local_mask  = sample_infections.generation .> 0
        local_cases = Float64(sum(local_mask .& (sample_infections.tinf .<= t_min)))
    end
    ...
    return (
        ...
        local_cases_at_detection   = local_cases,
...
function run_earliest_detection(;
    csv_path::String,
    output_path::String,
    num_samples::Int,
    ...
    println("="^80)
    println("EARLIEST DETECTION (single scenario)")
    println("="^80)
    ...
    length(mdts) != 1 &&
        error("Expected a single mean_detection_time; got $(length(mdts)) distinct values")
    ...
    # Build hariss_bg_cache once on the main process. Passed to workers via
    # CachingPool, so it is serialised at most once per worker (not per task).
    ...
    for batch_start in 1:batch_size:num_samples
        ...
        batch_rows = pmap(pool, batch_ids) do sample_id
            simulate_earliest_detection(
                country_trimmed, country, sample_id,
                R0, gen_time, icu_sampling_proportion,
                p_det, false_positive_rate, max_obs_time,
                hariss_bg_cache;
...
output_csv_path = joinpath(project_root, "global_model/pgfgleam/all_results/local/earliest_detection_result.csv")

run_earliest_detection(;
