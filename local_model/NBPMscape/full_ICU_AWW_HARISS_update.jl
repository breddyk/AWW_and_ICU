...
@everywhere function simulate_single_sample(
    ...
    for (idx, row) in enumerate(eachrow(country_data))
        ...
        # HARISS detection (throttled weekly)
        hariss_due = (time - last_hariss_check_time) >= 7.0 || time >= max_observation_time - 1.0
        if !hariss_detected && !isempty(sample_infections) && hariss_due
            ...
            try
                ...
                @warn "HARISS sampling failed" country=country_name sample=sample_id day=time err=hariss_err
            end
        end
        ...
        #   batched driver) + HARISS. HARISS detecting first does not end the day
        #   loop: imports and `simtree` still run until ICU and every p_det and
        #   HARISS have all fired. Compare `ICU_vs_WW_full_natural_history.jl`,
        #   which only waited on ICU + WW (not eight parallel AWW configs).
        # HARISS still consumes the growing `sample_infections` each weekly call.
        all_airport_detected = all(airport_detected[p] for p in airport_detection_probs)
        if icu_detected && all_airport_detected && hariss_detected
            break
        end
    end
