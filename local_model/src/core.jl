...
## Function to convert age range strings (e.g. "x0_4", "75plus") to Range type (e.g. 0:4)
function parse_range(str::String)
    # Clean up R formatting quirks ('x' prefixes, underscores, and 'plus')
    clean_str = replace(str, "x" => "", "X" => "", "_" => ":", "plus" => ":100")
    
    parts = parse.(Int, split(clean_str, ':'))
...
function initialize_parameters(config_file::String="")
	...
	if isfile( joinpath( project_root, "data/nhs_trust_site_sample_targets.csv" ) )
    	#println("File exists")
	else
    	println("Missing file required to load default parameters: data/nhs_trust_site_sample_targets.csv")
		return
	end
	if isfile( joinpath( project_root, "data/hariss_nhs_trust_sampling_sites.csv" ) )
    	#println("File exists")
	else
    	println("Missing file required to load default parameters: data/hariss_nhs_trust_sampling_sites.csv")
		return
	end
	if isfile( joinpath( project_root, "data/severity_care_probabilities/covid_knock_symptomatic_ihr_ifr_by_age.csv" ) )
    	#println("File exists")
	else
	...
	if isfile( joinpath( project_root, "data/severity_care_probabilities/covid_knock_care_pathway_prob_by_age.csv" ) )
    	#println("File exists")
	else
    ...
    if !isempty(config_file)
        #try 
            config_data = load_config( joinpath( project_root, config_file ) ); #config_data["parameters"]["symptomatic_IHR_IFR_by_age_file"] #config_data["parameters"]["care_pathway_prob_by_age_file"]
...
# function Infection(p; pid = "0", region="TLI3", tinf = 0.0, initialdow = 1, contacttype = :nothing, donor::Union{Nothing,Infection} = nothing)
	...
	D = length(dfargs) > 0 ? 
		...
		dfargs1 = [(u.pid, u.tinf, u.tgp, u.ted, u.thospital, u.ticu, u.tstepdown, u.tdischarge, u.trecovered, u.tdeceased, 
					u.severity, u.fatal, u.iscommuter, u.homeregion, u.commuteregion, u.generation,
					u.degree..., u.infectee_age, u.importedinfection) for u in G]

		Gdf = DataFrame(dfargs1
			, [:pid, :tinf, :tgp, :ted, :thospital, :ticu, :tstepdown, :tdischarge, :trecovered, :tdeceased, 
			:severity, :fatal, :iscommuter, :homeregion, :commuteregion, :generation, 
			:F, :G, :H, :infectee_age, :importedinfection ]
		)
