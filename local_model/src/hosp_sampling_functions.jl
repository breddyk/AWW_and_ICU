...
function sample_hosp_cases_n(; p = NBPMscape.P
    ...
    # ── LOCAL ADAPTATION: remap severity symbols to upstream category names ──
    # The local model uses :moderate_ED, :moderate_GP, :severe, :mildorasymptomatic.
    # The upstream tswab distribution dict expects :moderate_ED, :severe_hosp_short_stay,
    # :severe_hosp_long_stay. Map here so downstream code works without modification.
    if :severity in propertynames(hosp_cases)
        hosp_cases = copy(hosp_cases)
        hosp_cases[!, :severity] = map(hosp_cases[!, :severity]) do s
            if s == :severe
                :severe_hosp_long_stay
            elseif s == :moderate
                :moderate_ED          # fallback for old-style :moderate label
            else
                s                     # :moderate_ED, :moderate_GP already correct
            end
        end
    end

    # ── LOCAL ADAPTATION: add infectee_age placeholder if absent ──
    # The upstream NHS Trust assignment uses infectee_age to choose adult vs child probs.
    # Since the local model has no age, treat all cases as adults (age = 30).
    if !(:infectee_age in propertynames(hosp_cases))
        hosp_cases = copy(hosp_cases)
        hosp_cases[!, :infectee_age] = fill(Int8(30), nrow(hosp_cases))
    end
    ...
    # Uses ITL2_TO_NHS_TRUST_PROB_ADULT / _CHILD lookup tables. If the individual's homeregion
    # is not a column in the lookup (e.g. Wales codes), falls back to uniform sampling.
    hosp_nhs_trust_cd = Vector{Any}(undef, size(hosp_cases_Eng,1))
    for i in 1:size(hosp_cases_Eng,1)
        hr = hosp_cases_Eng[i,:].homeregion
        inf_age = hosp_cases_Eng[i,:].infectee_age
        lookup = inf_age < 16 ? ITL2_TO_NHS_TRUST_PROB_CHILD : ITL2_TO_NHS_TRUST_PROB_ADULT
        trust_codes = lookup[:, :NHS_Trust_code]
        if Symbol(hr) in propertynames(lookup)
            probs = lookup[:, Symbol(hr)]
        else
            # Fallback: uniform across available trusts
            probs = fill(1.0 / length(trust_codes), length(trust_codes))
        end
        hosp_nhs_trust_cd[i] = String( only( wsample( trust_codes, probs, 1) ) )
    end
