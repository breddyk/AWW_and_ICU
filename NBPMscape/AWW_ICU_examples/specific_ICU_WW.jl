...
@everywhere function simulate_country_detection(
    ...
    if verbose
        ...
        println("    WW detection probs: $(airport_detection_probs)")
    end
    ...
    if verbose
        for p_det in airport_detection_probs
            ...
            println("    p_det=$p_det: WW=$airport_pct% detected")
        end
...
function run_simulations_from_merged_csv(
    ...
    output_path::String = "results_ww_icu.csv",
    batch_size::Int = 125,
    selected_countries::Union{Vector{String}, Nothing} = nothing
)
    """
    ...
    println("="^80)
    println("WW + ICU SIMULATIONS (8 WW TYPES, BATCHED, SAVE EVERY $batch_size)")
    println("="^80)
    ...
    println("WW base p_det values: 4%, 16%")
    println("WW sampling fractions: 10%, 25%, 50%, 100%")
    println("Workers: $(nworkers())")
    println("Batch size: $batch_size combinations")
    
    if !isnothing(selected_countries)
        println("Selected countries: $(selected_countries)")
    else
        println("Processing: ALL countries")
    end
    println("="^80)
    ...
    if !isnothing(selected_countries)
        param_combinations = filter(row -> row.outbreak_country in selected_countries, param_combinations)
        println("After country filter: $(nrow(param_combinations)) combinations")
    end
    ...
    # SKIP PROBLEMATIC COMBINATIONS (if not already filtered by selected_countries)
    ...
    for batch_num in 1:num_batches
        ...
        batch_results = pmap(indexed_combinations) do idx_and_row
            ...
            try
                ...
                println("[$global_idx/$total_combinations] ✓ $country: ICU=$(round(icu_mean_time, digits=2))d")
...
# ============================================================================
# RUN THE SIMULATIONS - WITH SELECTED COUNTRIES
# ============================================================================
...
results = run_simulations_from_merged_csv(
    ...
    output_path = "/Users/reddy/AWW_and_ICU/pgfgleam/datasets/full_result_selected_countries.csv",
    batch_size = 200,
    selected_countries = selected_countries  # Add this parameter
)

println("\n✓ WW + ICU simulations complete!")